...
describe("notes API", () => {
  ...
  it("rejects invalid and unsupported notes", async () => {
    ...
    expect(await unsupportedResponse.json()).toEqual({
      error: "currency GBP is not supported",
    });
  });
  ...
  it("creates, lists, and rejects duplicate notes", async () => {
    ...
    const differentlyCasedDuplicateResponse = await post(
      JSON.stringify({
        ...input,
        currency: "EUR",
        serial: input.serial.toUpperCase(),
  ...
  it("deletes existing notes and reports missing notes", async () => {
    ...
    const createResponse = await post(
      JSON.stringify({ currency: "EUR", denomination: 10, serial }),
    );
    expect(createResponse.status).toBe(201);

    const deleteUrl = new URL(
      `${serial.toLowerCase()}?currency=eur&denomination=10`,
      baseUrl,
    );
    ...
    expect(await missingResponse.json()).toEqual({
      error: `note ${serial} (EUR 10) not found`,
    });
  });

  it("requires a complete note", async () => {
    const serial = "RR3223530574";
    const missingCurrencyResponse = await fetch(
      new URL(`${serial}?denomination=10`, baseUrl),
      { method: "DELETE" },
    );
    expect(missingCurrencyResponse.status).toBe(400);
    expect(await missingCurrencyResponse.json()).toEqual({
      error: "currency query parameter is required",
    });

    const missingDenominationResponse = await fetch(
      new URL(`${serial}?currency=EUR`, baseUrl),
      { method: "DELETE" },
    );
    expect(missingDenominationResponse.status).toBe(400);
    expect(await missingDenominationResponse.json()).toEqual({
      error: "denomination query parameter is required",
    });

    const invalidDenominationResponse = await fetch(
      new URL(`${serial}?currency=EUR&denomination=ten`, baseUrl),
      { method: "DELETE" },
    );
  ...
  it("deletes one exact note when a serial and currency are shared", async () => {
    ...
    const oneDollarResponse = await post(
      JSON.stringify({ currency: "USD", denomination: 1, serial }),
    );
    const twoDollarResponse = await post(
      JSON.stringify({ currency: "USD", denomination: 2, serial }),
    );
    expect(oneDollarResponse.status).toBe(201);
    expect(twoDollarResponse.status).toBe(201);

    const deleteOneDollarResponse = await fetch(
      new URL(`${serial}?currency=USD&denomination=1`, baseUrl),
      { method: "DELETE" },
    );
    expect(deleteOneDollarResponse.status).toBe(204);

    const deleteRemainingResponse = await fetch(
      new URL(`${serial}?currency=USD&denomination=2`, baseUrl),
      { method: "DELETE" },
    );

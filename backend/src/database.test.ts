...
describe("notes database schema", () => {
    it("creates the current schema for a new database", () => {
        ...
        expect(db.query<{ user_version: number }, []>("PRAGMA user_version").get()).toEqual({ user_version: 1 });
        db.query("INSERT INTO notes (serial, currency, denomination, created) VALUES (?, ?, ?, ?)")
            .run("A23456789A", "USD", 1, new Date().toISOString());
        expect(() => db.query("INSERT INTO notes (serial, currency, denomination, created) VALUES (?, ?, ?, ?)")
            .run("A23456789A", "USD", 2, new Date().toISOString())).not.toThrow();
    ...
    it("migrates legacy data and normalises its fields", () => {
        ...
        legacyDb.query("INSERT INTO notes (serial, currency, denomination, created) VALUES (?, ?, ?, ?)")
            .run("pa8124161759", "eur", 10, "2026-08-02T12:00:00.000Z");
        legacyDb.close();

        const db = openDatabase(path);

        expect(db.query("SELECT serial, currency, denomination, created FROM notes").all()).toEqual([{
            serial: "PA8124161759",
            currency: "EUR",
            denomination: 10,
    ...
    it("refuses to merge legacy records that collide after normalisation", () => {
        const { db: legacyDb, path } = createLegacyDatabase();
        const insert = legacyDb.query("INSERT INTO notes (serial, currency, denomination, created) VALUES (?, ?, ?, ?)");
        ...
        expect(unchangedDb.query("SELECT serial FROM notes ORDER BY id").all()).toEqual([
            { serial: "PA8124161759" },
            { serial: "pa8124161759" },
        ]);
        unchangedDb.close();

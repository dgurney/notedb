...
function notesTableExists(db: Database): boolean {
    return db.query("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'notes'").get() !== null;
}
...
function migrateLegacySchema(db: Database): void {
    const collisions = db.query<NormalisationCollision, []>(`
      SELECT
    ...
    if (collisions.length > 0) {
        const collidingNotes = collisions
            .map(({ serial, currency, denomination, count }) => `${serial} (${currency} ${denomination}, ${count} records)`)
            .join(", ");
        throw new Error(`cannot migrate notes database because serial normalisation would merge records: ${collidingNotes}`);
    }
...
function initialiseDatabase(db: Database): void {
    ...
    if (!hasNotesTable) {
        if (schemaVersion !== 0) {
            throw new Error(`notes database schema version is ${schemaVersion}, but the notes table is missing`);
        }
        createSchema(db);
        return;
    }
